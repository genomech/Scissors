...
def HarvestStats(
	...
	for Unit in Units:
		...
		Result["Estimated lib size|."] = [int(MarkDupStats["ESTIMATED_LIBRARY_SIZE"][i]) if MarkDupStats["ESTIMATED_LIBRARY_SIZE"][i] == MarkDupStats["ESTIMATED_LIBRARY_SIZE"][i] else 0 for i in LibCount]
...
def DaemonicPipe(
	...
	for Unit in Protocol["Units"]:
		...
		if (Unit["Stage"] == 5) and (not NoCall):
			# Base Recalibration
			BaseRecalibration(
		...
		if (Unit["Stage"] == 6) and (not NoCall):
			# Variant Calling
			HaplotypeCalling(
